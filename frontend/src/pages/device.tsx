...
export function DevicePage() {
  const systemId = Number(useParams().systemId); const deviceId = Number(useParams().deviceId); const queryClient = useQueryClient(); const { has } = useAuth()
  const device = useQuery({ queryKey: ["device", systemId, deviceId], queryFn: () => getDevice(systemId, deviceId), enabled: systemId > 0 && deviceId > 0 })
  ...
  const command = useMutation({ mutationFn: ({ id, state }: { id: number; state: boolean }) => createCommand(systemId, deviceId, id, { desired_state: state }), onSuccess: refresh })
