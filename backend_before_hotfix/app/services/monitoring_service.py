...
def _actuator_history_payload(
    ...
    *,
    ...
    baseline = next(
        (point for point in reversed(ordered_points) if point["recorded_at"] < start),
        None,
    )
    window_points = [
        point
    ...
    last_changed_at = window_points[-1]["recorded_at"] if window_points else (baseline["recorded_at"] if baseline else None)
    ordered_points = ([{"recorded_at": start, "state": baseline["state"]}] if baseline else []) + window_points
    ...
    # Nếu không có baseline trước window, khoảng đầu tiên vẫn là UNKNOWN.
...
async def get_device_actuator_history(
    ...
    *,
    ...
    reading_rows = await device_actuator_reading_rows(
        db, project_id=project_id, device_id=device_id, start=start, end=end
    )
    ...
    readings_by_actuator: dict[int, list[dict]] = defaultdict(list)
    for actuator_id, recorded_at, voltage_v, current_a, quality in reading_rows:
        readings_by_actuator[actuator_id].append({
            "recorded_at": recorded_at,
            "voltage_v": voltage_v,
            "current_a": current_a,
            "quality": quality,
    ...
    for actuator_id, actuator_public_id in actuator_ids:
        ...
        normalized_points, statistics = _actuator_history_payload(points, start=start, end=end)
        items.append(
            {
                "actuator_id": actuator_public_id,
                "points": normalized_points,
                "readings": readings_by_actuator[actuator_id],
                "gaps": [],
