    ...
    *,
    ...
    previous = (
        await db.execute(
            ...
            .where(
                ActuatorStateHistory.actuator_id.in_(actuator_ids),
                ActuatorStateHistory.recorded_at < start,
            )
            .distinct(ActuatorStateHistory.actuator_id)
            .order_by(
                ActuatorStateHistory.actuator_id,
                ActuatorStateHistory.recorded_at.desc(),
                ActuatorStateHistory.id.desc(),
            )
        )
    ).all()
    return sorted([*previous, *current], key=lambda row: (row[0], row[1]))


async def device_actuator_reading_rows(
    db: AsyncSession,
    *,
    project_id: int,
    device_id: int,
    ...
    return (
        await db.execute(
            select(
                ActuatorReading.actuator_id,
                ActuatorReading.recorded_at,
                ActuatorReading.voltage_v,
                ActuatorReading.current_a,
                ActuatorReading.quality,
            )
            .where(
                ActuatorReading.actuator_id.in_(actuator_ids),
                ActuatorReading.recorded_at.between(start, end),
            )
            .order_by(
                ActuatorReading.actuator_id,
                ActuatorReading.recorded_at,
